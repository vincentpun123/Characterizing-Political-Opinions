...
```{r Data Subsets - Demographic and ASK ALL}
####DATA PREPARATION####
...
```{r Factor Analysis and Scree Plot}
library(psych)
library(lessR)
...
```{r Factor Analysis with and without rotation}
#Factor Analysis----
...
1) Subset with Demographic Data 

2) Subset for EDA (Missing Values, Correlation), factor as numeric 
...
minmax_data_frame <- lapply(INSERTSUBSETDATA, FUN = minmaxnorm)
...
7) Interpret results, understand how certain certain political opinions are correlated. Multidimensional/complex, not just democrat/republican

Identify political opinion characteristics that stand out using PCA/FA, which may lead to segmentation/political stance 

8) Compare certain characteristics per demographic subgroup (i.e. education, race, identified political party (PROBABLY NOT REQUIRED FOR THIS ASSIGNMENT, SINCE NO CLUSTERING). JUST IDENTIFY POLITICAL OPINIONS 

9) Next steps would be to run clustering techniques, perhaps 4 separate clusters, perhaps based on economic, foreign policy, nationalist viewpoints, etc 
...
```
**pewdata_sub_q_ALL_numframe_normalize_reduced_2**

```{r Corrplot - pewdata_sub_q_ALL_numframe_normalize_reduced_2}
...
```{r PCA - After Dimension Reduction}
...
```{r fa.parallel - After Dimension Reduction }
